...
const landlordsReducer = (
  state = { isLoading: true, landlords: [] },
  action,
) => {
  switch (action.type) {
    case START_LOADING:
    ...
    case UPDATE_LANDLORD:
      return {
        ...state,
        landlords: state.landlords.map((lan) =>
          lan._id === action.payload._id ? action.payload : lan,
        ),
      };
    case DELETE_LANDLORD:
      return {
        ...state,
        landlords: state.landlords.filter((lan) => lan._id !== action.payload),
      };
    default:
      return state;
  }
